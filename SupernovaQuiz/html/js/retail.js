...
function startProgram(){
	...
	session.service("ALAnimatedSpeech").done(function (tts){
		tts.say("Welcome to the Brubotics Quiz");
	});
}
...
function startQuiz(){
	...
	session.service("ALAnimatedSpeech").done(function (tts){
		tts.say("Would you like to take a fun little quiz?");
	});
}
...
function nextQuestion(){
	...
	questionCount +=1
	// IF IT IS LESS THAN 6 QUESTIONS, SHOW THE NEXT ONE, IF NOT SHOW  THE QR PART
	if(questionCount<=5){
		document.getElementById("feedback-view").style.display="none";			
		document.getElementById("question-view").style.display="block";
...
function showResults(){
	...
	document.getElementById("correct-ans").innerHTML = "You answered correcty " + correctAns.toString() + " out of 6 questions";
	session.service("ALAnimatedSpeech").done(function (tts){
	    tts.say("You answered correctly " + correctAns.toString() + " out of 6 questions");
